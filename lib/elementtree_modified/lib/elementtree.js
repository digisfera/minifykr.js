...
Element.prototype.delete = function(key) {
  ...
  if (options.method === "text") {
    ...
    if (options.method === "xml") {
      _serialize_xml(function(data) {
        sb.push(data);
      }, this._root, options.encoding, qnames, namespaces, indent, indent_string, options.minify);
    }
    else {
      /* TODO: html */
...
function _serialize_xml(write, elem, encoding, qnames, namespaces, indent, indent_string, minify) {
  var tag = elem.tag;
  var text = elem.text;
  var tail = elem.tail;
  var parentType = elem.parentType;
  var items;
  var i;
  ...
  if ( (!minify || parentType == 'data' || parentType == 'action') && tag === Comment ) {
    ...
    if (tag === undefined) {
      ...
      elem.iter(function(e) {
        _serialize_xml(write, e, encoding, qnames, null, newlines ? indent + 1 : false, indent_string, minify);
      });
    }
    else {
      write("<" + tag);
  ...
  if(tail !== null){
    if(parentType == 'data' || parentType == 'action'){
      writeTail = tail;
    }
    else{
      writeTail = minify ? '' : tail;
    }
