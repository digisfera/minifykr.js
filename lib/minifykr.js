...
function minifyElement(e) {
  if(e.tag != "data" && e.tag != "action" && e.tag != elementtree.Comment) { e.text = "";}
  else {
    e.krpano = true;
  }
  return e;
    ...
    function concatenateAux(e, opts) {
      ...
      if(e.tag !== "include") {
        ...
        if(minifyType === 'false') {
          ...
          minifyKrFile(includedFile, outputFilename, opts.encrypt, opts.minify, function(err, outputFile) {
          ...
          toReturn = includedRoot.getchildren().map(function(e) {
          ...
          if(toReturn.length > 0){
            toReturn[toReturn.length - 1].tail = e.tail;
          }
...
function rootNodeToXML(root, shouldMinify) {
  var minTree = new ElementTree(root);
  return minTree.write({ encoding: 'utf-8', xml_declaration: false, minify: shouldMinify });
}
...
function minifyKr(inputData, opts, callback) {
    try {
      ...
      root = opts.minify ? minify(root) : root;
    }
    catch(e) {
      return callback(e);
    }

    callback(null, rootNodeToXML(root, opts.minify));
}
...
function generateOutputFilename(filename) {
 ...
 function minifyKrFile(inputFile, outputFile, encrypt, shouldMinify, callback) {
  inputFile = inputFile || "main.xml";
  if(encrypt === undefined) { encrypt = true; }
  if(shouldMinify === undefined) { shouldMinify = true; }
